<h1 class="text-2xl font-bold mb-6 text-gray-800">
    {{ $Modo == 'crear' ? 'Agregar dietas' : 'Modificar dietas' }}
</h1>
<br>

<form action="{{ $Modo == 'crear' ? url('dietas') : url('dietas/'.$dieta->id_dieta) }}" method="POST" class="w-[500px] mx-auto p-6 bg-white shadow-md rounded-lg">
    @csrf
    @if($Modo == 'editar')
        @method('PUT')
    @endif
...
<div class="mb-6">
    ...
    <textarea name="descripcion" id="descripcion" rows="4"
              class="form-control mt-2 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm resize-y {{ $errors->has('descripcion') ? 'is-invalid' : '' }}">{{ old('descripcion', $dieta->descripcion ?? '') }}</textarea>
...
<div class="mb-6">
    ...
    <input type="date" name="fecha_inicio" id="fecha_inicio"
           value="{{ old('fecha_inicio', $dieta->fecha_inicio ?? '') }}"
...
<div class="mb-6">
    ...
    <input type="date" name="fecha_fin" id="fecha_fin"
           value="{{ old('fecha_fin', $dieta->fecha_fin ?? '') }}"
    ...
    <div class="flex justify-between mt-4">
        ...
        <a href="{{url('administracion/dietas')}}" class="inline-block bg-gray-300 text-gray-700 px-6 py-3 rounded-md hover:bg-gray-400 transition duration-300">Volver</a>
    </div>
</form>
